...
- [ ] **QR.3 [AGENT] - REOPENED (2026-08-18): the Rust command was LOST in a revert.**
  The JS side survived (`settings.js` `showLinkCode()` calls `bridge("desktop_link_code")`,
  the "Show linking QR" button) and so did the permission
  (`desktop/permissions/autogenerated/desktop_link_code.toml`), but `desktop_link_code`
  and `build_link_payload` are GONE from `desktop/src/lib.rs` (0 refs; `git grep HEAD`
  confirms they are not in the committed lib.rs). So the button currently errors
  "command not found." RE-ADD the Rust command to the `#[cfg(desktop)]` module exactly per
  the pinned wire format below, register it in the desktop `generate_handler!`, and
  confirm the existing `desktop_link_code.toml` + `home-links` capability still reference
  it. Then re-verify (reveal the QR, decode it). Original task text follows:
