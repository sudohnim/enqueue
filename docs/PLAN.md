...
- [ ] **MOBBOOT.1 [AGENT]** Cold-launch race: bootstrap() shows the SETUP screen even when the phone is configured with 73 synced notes. Found during MOBRENDER.1 device-verify: after a cold launch the library was stuck on "Scan the linking QR", but CDP showed `mobile_status` = configured:true and calling `bootstrap()` again immediately rendered the library (4 shelves). So the data is fine; bootstrap just decided "setup" too early and never retried.
