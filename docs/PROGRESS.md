...
- **MOBFIX.4** ✅ Link add-option: two-field form (URL + notes). URL input (type=url, inputmode=url) + notes textarea (rows=3). https:// prepend for scheme-less URLs. Payload: url + '\n\n' + notes to mobile_capture. Purple Save button, back navigation. bin/verify green.
