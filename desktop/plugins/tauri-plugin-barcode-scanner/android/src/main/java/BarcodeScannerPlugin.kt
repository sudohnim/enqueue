...
class ScanOptions {
    ...
    var boxSize: Int? = null  // Size of the square box in dp (e.g., 280)
}
...
class BarcodeScannerPlugin(private val activity: Activity) : Plugin(activity),
    ...
    private fun setupCamera(cameraDirection: String, windowed: Boolean, boxSize: Int = 0) {
        activity
            .runOnUiThread {
                // Calculate box dimensions in pixels
                val boxSizePx = if (boxSize > 0) {
                    (boxSize * activity.resources.displayMetrics.density).toInt()
                } else {
                    // Default to 280dp if not specified
                ...
                if (windowed && boxSize > 0) {
                    // Windowed mode with constrained box
                    previewParams = FrameLayout.LayoutParams(boxSizePx, boxSizePx)
                    previewParams.gravity = android.view.Gravity.CENTER
                } else {
                    previewParams = FrameLayout.LayoutParams(
                ...
                if (windowed && boxSize > 0) {
                    ...
                    overlayParams = FrameLayout.LayoutParams(boxSizePx, boxSizePx)
                ...
                graphicOverlay.layoutParams = overlayParams
    ...
    private fun prepare(direction: String, windowed: Boolean, boxSize: Int = 0) {
        dismantleCamera()
        setupCamera(direction, windowed, boxSize)
    }
    ...
    fun scan(invoke: Invoke) {
        ...
        if (hasCamera()) {
            if (getPermissionState("camera") != PermissionState.GRANTED) {
                ...
                webViewBackground = null
                prepare(args.cameraDirection ?: "back", args.windowed, args.boxSize ?: 0)
