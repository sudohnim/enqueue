...
function restorePill(where) {
	...
	pill.innerHTML = html;
	// The ask button wears the living eye (EYE.2): one factory injects the same
	// markup and the shared EYE.1 pair tracks it, so re-rendering the pill on
	// every navigation never stacks listeners. Null-guarded: makeEye no-ops
	// when the eye container is absent. The travel math self-scales from the
	// socket's rendered size, so no size-specific tuning is needed here.
...
function toggleMenu(e) {
	...
	menu.addEventListener("keydown", (ev) => {
		...
		if (ev.key === "ArrowDown" || ev.key === "ArrowUp") {
			ev.preventDefault();
			const next =
				(at + (ev.key === "ArrowDown" ? 1 : -1) + items.length) % items.length;
			items[next].focus();
		}
		if (ev.key === "Home") {
			ev.preventDefault();
