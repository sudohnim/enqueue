...
// Eager clipboard write. WKWebView (the Tauri webview) puts copied text on the
// pasteboard as lazy/promised data - the bytes are not materialised until a
// consumer pulls them. A real paste pulls, so Cmd+V works; but a clipboard
// manager (Paste, Maccy, ...) snapshots the pasteboard the instant changeCount
// bumps and reads eagerly, so it captures an empty promise and the copy never
// lands in history. Writing the selection ourselves in the copy handler forces
// the bytes onto the pasteboard now, so managers see it too. Plain-text and
// HTML both, so rich targets keep formatting. No selection (e.g. a Cmd+C inside
// an <input>/<textarea>, whose selection this API does not see) -> let the
// native copy proceed untouched.
