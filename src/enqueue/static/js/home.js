  ...
  async function home(opts) {
    ...
    let html =
      ...
      // groups the socket and the frame so the iris stays clipped inside it.
  ...
  // interaction of its own and is hidden from the accessibility tree. It leans
  // toward the cursor (the iris slides inside the fixed socket) and dies with
  // the view - teardown() clears the listeners - so no other surface inherits a
  // stray handler.
  let eyeMove = null;
  let eyeLeave = null;
  ...
  function mountEye() {
    ...
    if (!el) return;
    const layer = el.querySelector(".eye-pupil");
    // The follow is functional, not decorative (L.7/M.4), so it runs even under
    // prefers-reduced-motion. The frame (socket: outline, lashes, ground) never
    // moves - only the purple iris pupil slides inside the clipped socket (O.3),
    // a real follow rather than a whole-eye fake. The travel scales with the
    // socket's display size.
    let raf = 0;
    let rafQueued = 0;
    const rest = () => {
      if (raf) {
